...
plates_a = plates(thesebatchplates,:); % subset 4lines2K plates b1

batch='4L2K_3';
thesebatchplates = ismember(plates.batch,batch);
plates_b = plates(thesebatchplates,:);% subset 4lines2K plates b2

batch='4L2K_5';
thesebatchplates = ismember(plates.batch,batch);
plates_c = plates(thesebatchplates,:);% subset 4lines2K plates b3

plates = vertcat(plates_a,plates_b,plates_c); %all 3 4lines2K batches
...
cellplates = cellplates(cellplates.CellLine == 'A549_XRCC5',:); %try on one cell line
...
%Pick which features subset you want to analyze
featureset = use_all; %NBT %use_2ch %use_all 

% Subset features
plate.feaInfo.name = plate.feaInfo.name(featureset);
plate.feaInfo.channel_id = plate.feaInfo.channel_id(featureset);
plate.feaInfo.category_id = plate.feaInfo.category_id(featureset);
for w = 1:size(plate.profiles,1)
    plate.profiles{w,1} = plate.profiles{w,1}(featureset);
end

clear name category_id channel_id w

%% Check feature set in subsetted plate
% reassign to remove parts of struct that cant convert to table
plate_use_2ch.name = plate.feaInfo.name';
plate_use_2ch.category_id = plate.feaInfo.category_id';
plate_use_2ch.channel_id = plate.feaInfo.channel_id';
plate_use_2ch = struct2table(plate_use_2ch); % create table listing features used to be sure no YFP features used
writetable(feature_table,[results_folder,'\featuresused_2ch.xlsx'])
...
for p=1:height(cellplates)
    % Load single plate
    plate_IDs = cellplates.expt_plate;
    plate_ID = plate_IDs(p);
    display(plate_ID);
    plate = Load_Batch_LE(cellplates,plate_ID, profiles_folder,qc_folder);
    % subset single plate
    plate.feaInfo.name = plate.feaInfo.name(featureset);
    plate.feaInfo.channel_id = plate.feaInfo.channel_id(featureset);
    plate.feaInfo.category_id = plate.feaInfo.category_id(featureset);
    ...
    [clfy_obj, accuracy, cv_report, scan_scheme] = Classifier(trainData, trainLabels,classifier_type,'XtrainNames',trainDataInfo,'cv_fold',kfold,...
        ...
        'NumNeighbors',3);
    % Record Results
    classifier_pp.accuracy{p} = accuracy;
    classifier_pp.clfy_obj{p} = clfy_obj;
    classifier_pp.cv_report{p} = cv_report;
    classifier_pp.scan_scheme{p} = scan_scheme;
end
save(fullfile(results_folder,'classifier_perplate.mat'),'classifier_pp')

% Calc CA for any 2 plates pooled (so example training data in dose/class)
% define all combos lof 6 plates to pool
combos = cell(15,1);
for x = 1:5
    combos{x,1} = cellplates([1 x+1],{'expt_plate','plate_type'});
end
for y = 3:6
    combos{y+3,1} = cellplates([2 y],{'expt_plate','plate_type'});
end
for z = 4:6
    combos{z+6,1} = cellplates([3 z],{'expt_plate','plate_type'});
end
for w = 5:6
    combos{w+8,1} = cellplates([4 w],{'expt_plate','plate_type'});
end
combos{15,1} = cellplates([5 6],{'expt_plate','plate_type'});
...
for c=1:length(combos)
    ...
    [clfy_obj, accuracy, cv_report, scan_scheme] = Classifier(trainData, trainLabels,classifier_type,'XtrainNames',trainDataInfo,'cv_fold',kfold,...
        'FDA_type',FDA_type,'gamma',gamma,'kernel_type',kernel_type,'kernel_para',kernel_para,...
        'NumNeighbors',3);
    classifier_pw.accuracy{c} = accuracy;
    classifier_pw.clfy_obj{c} = clfy_obj;
    classifier_pw.cv_report{c} = cv_report;
    classifier_pw.scan_scheme{c} = scan_scheme;
end
save(fullfile(results_folder,'classifier_pairwise.mat'),'classifier_pw')
...
labelpcplot()
saveas(gcf,[results_folder,'\','XRCC5','ch1_3','PCA']); % change naming by featureset! 
...
saveas(gcf,[results_folder,'\','XRCC5','ch1_3','FDA']);
...
saveas(gcf,[results_folder,'\','XRCC5','ch1_3','clustering'])
...
saveas(gcf,[results_folder,'\','XRCC5','ch1_3','cluster_purity'])
