...
for m = 1:height(plates) % for all plates in this cell line
    plate_maps{m} = [num2str(plates.expt_plate{m}),'.xlsx']; % find the platemap names
end
...
for p = 1:height(plates) % for all plates
    ...
    Get_KS_ProfilesInCell(plateID, plate_type, well_names, drug_names, drug_categories, concentrations,...
